...
namespace FirstGearGames.LobbyAndWorld.Lobbies
{
    public class LobbyNetwork : NetworkBehaviour
    {
        ...
        public event Action<NetworkObject> AddedAvatarUrl;
        ...
        public Dictionary<NetworkConnection, string> AvatarUrlNo = new Dictionary<NetworkConnection, string>();
        #endregion
        ...
        private void FirstInitialize()
        {
            _instance = this;
            //Find LobbyCanvases.
            LobbyCanvases = GameObject.FindAnyObjectByType<LobbyCanvases>();
       ...
       //     LoggedInUsernames.Remove(obj);
        ...
        private void ClientReset()
        {
            ...
            //    ci.PlayerSettings.SetUsername(string.Empty);
                ci.PlayerSettings.SetAvatarurl(string.Empty);
        }
        ...
        private void CmdAddUrl(string url, NetworkConnection sender = null)
        {
            ClientInstance ci;
            if (!FindClientInstance(sender, out ci))
                return;

            AvatarUrlNo[ci.Owner] = url;
            ci.PlayerSettings.SetAvatarurl(url);
            AddedAvatarUrl?.Invoke(ci.NetworkObject);
        }
        ...
        private void LeaveRoomInternal()
        {
          ...
          // var roomdetails =  ReturnRoomDetails("ava");
        ...
        private void ServerLoadedScene(SceneLoadEndEventArgs args)
        {
            ...
            SceneLoadData sld = new SceneLoadData(args.LoadedScenes)
            {
                ...
                PreferredActiveScene = new PreferredScene(new SceneLookupData(args.LoadedScenes[0]), null),
            };
            //Load connections in.
        ...
        private RoomDetails RemoveFromRoom(NetworkObject clientId, bool clientDisconnected)
        {
            ...
            if (roomDetails != null)
            {
                ...
                if (_instance != null && _instance.TryGetPlayerAvatar(clientId.Owner, out NetworkObject playerAvatar))
                {
                    if (playerAvatar != null)
                    {
                        playerAvatar.Despawn();
                    }
                }
        ...
        {
            SampleManger sm = FindObjectOfType<SampleManger>();
            if (sm != null)
            {
                return sm.TryGetPlayerAvatar(conn, out playerAvatar);
            }
            else
            {
                playerAvatar = null;
